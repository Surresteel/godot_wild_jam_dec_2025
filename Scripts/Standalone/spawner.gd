...
const spawn_radius_min_iceberg: float = 20.0
#const spawn_radius_min_iceberg: float = 100.0
const spawn_radius_max_iceberg: float = 50.0
#const spawn_radius_max_iceberg: float = 200.0
const MAX_COUNT_ICEBERGS: int = 50
...
@export var _interval_iceberg: float = 15.0
...
#func _cleanup_icebergs() -> void:
#	for i in range(_icebergs.size() - 1, -1, -1):
#		if not _icebergs[i]:
#			_icebergs.remove_at(i)
...
func _spawn_iceberg(max_layer_size: int = IcebergBase.HIGHEST_LATER, 
		pos_override: Variant = null) -> void:
	#if _icebergs.size() >= MAX_COUNT_ICEBERGS:
	#	return
