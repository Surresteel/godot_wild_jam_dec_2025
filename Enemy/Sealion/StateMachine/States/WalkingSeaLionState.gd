...
func enter(sealion: Sealion) -> void:
	...
	sealion.change_target(sealion.get_tree().get_first_node_in_group("Player")) #TODO the sealion should go to targeting to get a new target, either the player or the helmsmen and maybe the baby penguins
...
func exit(sealion: Sealion) -> void:
	#Set NavigationAgentLayer
	sealion.nav_agent.set_navigation_layer_value(1,false)
