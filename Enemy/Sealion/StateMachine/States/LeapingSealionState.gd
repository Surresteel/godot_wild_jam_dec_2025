...
func update(sealion: Sealion, delta) -> void:
	sealion.velocity.x = move_toward(sealion.velocity.x, sealion.ship.linear_velocity.x, delta * 0.5)
	sealion.velocity.z = move_toward(sealion.velocity.z, sealion.ship.linear_velocity.z, delta * 0.5)
